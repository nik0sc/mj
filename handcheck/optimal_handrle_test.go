...
func Test_OptHandRLEChecker_Check(t *testing.T) {
	tests := []struct {
		name string
		hand string
		want Result
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rFast := OptHandRLEChecker{Split: true, UseMemo: true}.Check(h)
			// order may be wrong
			...
			rSlow := OptHandRLEChecker{}.Check(h)
...
func Benchmark_OptHandRLEChecker_AllP(b *testing.B) {
	hand, _ := mj.ParseHand("b1 b1 b1 b1 b1 b1 b1 b1 b1 b1 b1 b1 b1 b1")
	benchmark_OptHandRLEChecker(b, hand)
}

func Benchmark_OptHandRLEChecker_AllPReal(b *testing.B) {
	hand, _ := mj.ParseHand("b1 b1 b1 b2 b2 b2 b3 b3 b3 b4 b4 b4 b5 b5")
	benchmark_OptHandRLEChecker(b, hand)
}

func Benchmark_OptHandRLEChecker_AllC(b *testing.B) {
	hand, _ := mj.ParseHand("b1 b2 b3 b3 b4 b5 b5 b6 b7 b7 b8 b9 b9 b9")
	benchmark_OptHandRLEChecker(b, hand)
}

func Benchmark_OptHandRLEChecker_NS(b *testing.B) {
	hand, _ := mj.ParseHand("w1 b7 w4 c5 b9 he w5 hf w5 c3 b8 hf hn hf")
	benchmark_OptHandRLEChecker(b, hand)
}

func benchmark_OptHandRLEChecker(b *testing.B, h mj.Hand) {
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_ = OptHandRLEChecker{UseMemo: true}.Check(h)
	}
}
